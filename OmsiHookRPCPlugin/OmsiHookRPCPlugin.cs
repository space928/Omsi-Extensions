...
public class OmsiHookRPCPlugin
{
    ...
    private static readonly List<Thread> threadPool;
    private static readonly ConcurrentQueue<MethodData> callQueue;
    private static readonly ConcurrentBag<NamedPipeServerStream> pipes;
    private static readonly ArrayPool<byte> argumentArrayPool;
    private static readonly List<ReturnData> returnPool;
    private static readonly object logLock = new();
    private static readonly object writeMutex = new();

    static OmsiHookRPCPlugin()
    {
        threadPool = [];
        callQueue = [];
        pipes = [];
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)], EntryPoint = nameof(PluginStart))]
    // ReSharper disable once UnusedMember.Global
    public static void PluginStart(nint aOwner)
    {
        try
        {
    ...
    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)], EntryPoint = nameof(AccessTrigger))]
    public static void AccessTrigger(ushort variableIndex, [C99Type("__crt_bool*")] nint triggerScript) { }

    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static void AccessStringVariable(ushort variableIndex, [C99Type("char*")] nint firstCharacterAddress, [C99Type("__crt_bool*")] nint writeValue) { }

    [UnmanagedCallersOnly(CallConvs = [typeof(CallConvCdecl)])]
    public static void AccessSystemVariable(ushort variableIndex, [C99Type("float*")] nint value, [C99Type("__crt_bool*")] nint writeValue) { }
    ...
    private readonly struct MethodData(RemoteMethod method, byte[] args, int threadId, int returnPromise)
    {
        public readonly RemoteMethod method = method;
        public readonly byte[] args = args;
        public readonly int threadId = threadId;
        public readonly int returnPromise = returnPromise;
    }
}
